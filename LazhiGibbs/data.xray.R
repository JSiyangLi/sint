...
xray.data <- function(
    ...
    rmat_file, # the .txt file of r matrix
...
)
{
  ...
  # read in the data
  df <- read.table(file = count_file)$V2 # * 1e5 # temporary scaling
  ...
  Rmat <- read.table(file = rmat_file)[-1, ]
  ...
  sources$n      <- ncol(Rmat) # number of sources
  seg$n.overlap  <- rowSums(Rmat!=0)
  seg$area       <- Aroi
  seg$eff.area   <- Exposures
  seg$maxoverlap <- max(seg$n.overlap)
  seg$cnt.obs <- segment.count
  
  src.contents <- which(t(Rmat) != 0) %% sources$n # preparing contents to be filled into the src matrix
  src.rows <- which(t(Rmat) != 0) %/% sources$n # preparing the rows the contents to be filled in
  src.whole <- src.contents == 0
  src.contents[src.whole] <- sources$n # the 0 in whole division to be converted to the dividant
  src.rows[!src.whole] <- src.rows[!src.whole] + 1
  src.r <- t(Rmat)[t(Rmat) != 0]
  
  seg$r <- seg$src.index <- matrix(rep(0, (seg$n)*(seg$maxoverlap)), ncol = seg$maxoverlap)
  for (i in unique(src.rows)) {
    src.len <- length(src.contents[src.rows == i])
    seg$src.index[i, 1:src.len] <- src.contents[src.rows == i]
    seg$r[i, 1:src.len] <- src.r[src.rows == i]
    }
