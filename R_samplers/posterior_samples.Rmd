...
py_discover_config(required_module = "emcee")
...
D = np.array(r.simulator(n = n, Lambda_stars = Lambda_stars, xi_star = xi_star))
...
r.tic()
emcee_result = emcee_sampler.run_mcmc(t_init, nsteps = 20000, progress = True,
...
# emcee sample plots
##############
true_values <- c(py$t_mu, py$t_theta, py$t_pid, py$t_xi, log(py$Lambda_stars / (R * e * Time)))
...
plot.new()
plot.window(xlim = c(0, 1), ylim = c(0, 1))
text(0.5, 0.75, "basic model", font = 2, cex = 1.1)
dev.off()
...
def loglikelihood_dynesty(zeta):
    ...
    #aux = zeta[4]
    Lambda = zeta[4:]
    return r.loglik(t_xi, Lambda, D, A, float(Time), a, R, e)
...
def int_loglikelihood_dynesty(zeta):
    ...
    pid = zeta[2]
    xi = zeta[3]
    return r.integrated_likelihood(mu, theta, pid, xi, D, A, float(Time), a, R, e)
...
dynesty_result = sampler.run_nested(dlogz=0.1)
...
plot.new()
plot.window(xlim = c(0, 1), ylim = c(0, 1))
text(0.5, 0.75, "marginal model", font = 2, cex = 1.1)
dev.off()
```

```{python three_sampler_tests}
import emcee
from tqdm import tqdm
import h5py
...
Lazhi_obs <- simulator(n = 1000, Lambda_stars = Lazhi_Lambda_star, xi_star = xi_star)
...
comparison_original_Lazhi_qq <- function(i) {
  qqplot(emcee_thin_flat[, i], Lazhi_mcmc[, i], xlab = "basic model on emcee", ylab = "basic model on Gibbs(Lazhi)", pch = 20, las = 1, main = paste("Q-Q plot of", param_name[i]))
...
sink()
#################################
# Lazhi sampler vs. marginal model emcee
comparison_marginal_Lazhi_qq <- function(i) {
  qqplot(emcee_int_thin_flat[, i], Lazhi_mcmc[, i], xlab = "marginal model on emcee", ylab = "basic model on Gibbs(Lazhi)", pch = 20, las = 1, main = paste("Q-Q plot of", param_name[i]))
  abline(a = 0, b = 1, col = "red")
}
...
#HDFbackend = emcee.backends.HDFBackend("emcee_chains")
r.tic()
emcee_int_result = emcee_int_sampler.run_mcmc(t_int_init, nsteps = 20000, progress = True,
  skip_initial_state_check = True)
...
comparison_qq <- function(i) {
  qqplot(emcee_thin_flat[, i], emcee_int_thin_flat[, i], xlab = "basic model", ylab = "marginal model", pch = 20, las = 1, main = paste("Q-Q plot of", param_name[i]))
  abline(a = 0, b = 1, col = "red")
}
pdf("simple/integrated_emcee/joint_emcee_results/QQplots.pdf")
sapply(1:4, comparison_qq)
dev.off()
sink(file = paste0(getwd(), "/simple/integrated_emcee/joint_emcee_results/KStest.txt"))
sapply(1:4, function(i) ks.test(emcee_thin_flat[, i], emcee_int_thin_flat[, i]))
sink()

# emcee's vs dynesty
comparison_dyem <- function(i) {
  qqplot(emcee_thin_flat[, i], t_dynesty_sample[, i], xlab = "basic model on emcee", ylab = "basic model on dynesty", pch = 20, las = 1, main = paste("Q-Q plot of", param_name[i]))
  abline(a = 0, b = 1, col = "red")
}
comparison_int_dyem <- function(i) {
  qqplot(emcee_int_thin_flat[, i], t_dynesty_sample[, i], xlab = "marginal model on emcee", ylab = "basic model on dynesty", pch = 20, las = 1, main = paste("Q-Q plot of", param_name[i]))
  abline(a = 0, b = 1, col = "red")
}
pdf("simple/integrated_emcee/joint_emcee_results/dyemQQplots.pdf")
sapply(1:4, comparison_dyem)
dev.off()
pdf("simple/integrated_emcee/joint_emcee_results/dy_intemQQplots.pdf")
...
comparison_intdyem <- function(i) {
  qqplot(emcee_thin_flat[, i], t_int_dynesty_sample[, i], xlab = "basic model on emcee", ylab = "basic model on dynesty", pch = 20, las = 1, main = paste("Q-Q plot of", param_name[i]))
  abline(a = 0, b = 1, col = "red")
}
comparison_int_intdyem <- function(i) {
  qqplot(emcee_int_thin_flat[, i], t_int_dynesty_sample[, i], xlab = "marginal model on emcee", ylab = "basic model on dynesty", pch = 20, las = 1, main = paste("Q-Q plot of", param_name[i]))
  abline(a = 0, b = 1, col = "red")
}
pdf("simple/integrated_emcee/joint_emcee_results/intdy_emQQplots.pdf")
sapply(1:4, comparison_intdyem)
dev.off()
pdf("simple/integrated_emcee/joint_emcee_results/intdy_intemQQplots.pdf")
...
comparison_dy_intdy <- function(i) {
  qqplot(t_int_dynesty_sample[, i], t_dynesty_sample[, i], xlab = "marginal model on dynesty", ylab = "basic model on dynesty", pch = 20, las = 1, main = paste("Q-Q plot of", param_name[i]))
  abline(a = 0, b = 1, col = "red")
}
...
RDSlist <- list("simulated_data" = py$D,
                ...
                "dynesty_result" = py$dres,
                "dynesty_sample" = py$emcee_sample)
saveRDS(RDSlist, file = paste0(getwd(), "/simple/emcee/results/emcee_result.RDS"))
```
...
qqplot(analytical_integral_evaluation, numerical_integral_evaluation, pch = 20,
       ...
       main = "marginal model evaluated at a prior sample",
       sub = "zoom in",
