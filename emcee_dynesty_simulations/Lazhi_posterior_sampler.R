...
Lazhi_simulator <- function(iterations = 1000, t_init = c(t_mu, t_theta, t_pid, t_xi),
  ...
  lQ <- function(lparams, Lambda) {
    ...
    lp + lmid + llast# - lmu - ltheta # the last two terms are Jacobian, added to lp
  }
  ...
  while (it <= (iterations + 1)) {
    ...
    # step 1
    #########
    resulting_sample[it, 3] <- rbeta(n = 1, shape1 = nd[it - 1] + 1, shape2 = n - nd[it - 1] + 1)
...
marginalised_gibbs_sampler <- function(iterations = 1000, t_init = c(t_mu, t_theta, pid, t_xi),
                            D, A, Time, a, r, e,
                            alpha0 = 1e-6, beta0 = 1.1, 
                            #n_mh_iter = 100,
                            #emg_lambda = 1e-1,
  ...
  # internal functions
  ####################
  zero_inf_negbinom_likelihood <- function(lmu, ltheta, pid, Signal, Signal_0_len = sum(which(Signal == 0))) {
    Signal_pos <- Signal[Signal > 0]
    
    # zero part
    alpha <- exp(2 * lmu - ltheta)
    lbeta <- lmu - ltheta
    ldenom <- logplus(lbeta, log(r) + log(e) + log(Time))
    lprob_param <- alpha * (lbeta - ldenom)
    zero_lik <- logplus(log(pid), log(1 - pid) + lprob_param) * Signal_0_len
    
    # positive part
    tot_pos_lik <- sum(dnbinom(x = Signal_pos, size = alpha, prob = exp(lprob_param), log = TRUE))
    pos_lik <- logplus(tot_pos_lik, log(1 - pid))
    sum(zero_lik, pos_lik)
  }
  lhyperparam_kernel <- function(lparams, pid, Signal) {
    lmu = lparams[1]
    ltheta = lparams[2]
    lik <- zero_inf_negbinom_likelihood(lmu, ltheta, pid = pid, Signal = Signal)
    lik + t_logmuhypprior(lmu) + t_logthetahypprior(ltheta)
  }
  ...
  while (it <= (iterations + 1)) {
    ...
    opt_list <- optim(par = resulting_sample[it - 1, 1:2],
                      fn = lhyperparam_kernel, method = "BFGS", control = list(fnscale = -1), hessian = TRUE, pid = pid, Signal = Signal)
    tau <- opt_list$par
    Sigma <- -ginv(opt_list$hessian)
    ...
    #lacceptance_ratio <- rep(NA, iterations)
    proposal <- rmvnorm(n = 1, mean = tau, sigma = Sigma)
    lacceptance_ratio <- lhyperparam_kernel(lparams = proposal, pid = resulting_sample[it, 3], Signal = Signal) +
      dmvnorm(x = resulting_sample[it - 1, 1:2], mean = tau, sigma = Sigma, log = TRUE) - 
      lhyperparam_kernel(lparams = resulting_sample[it - 1, 1:2], pid = resulting_sample[it, 3], Signal = Signal) -
      dmvnorm(x = proposal, mean = tau, sigma = Sigma, log = TRUE)
    #lacceptance_prob <- min(0, lacceptance_ratio)
    accepted <- log(runif(1)) < lacceptance_ratio
    ...
    ########
    # step 5: nd calculation
    ########
    # 0 signal length
    Signal_0_len = sum(which(Signal == 0))
    
    # zero part
    alpha <- exp(2 * resulting_sample[it, 1] - resulting_sample[it, 2])
    lbeta <- resulting_sample[it, 1] - resulting_sample[it, 2]
    ldenom <- logplus(lbeta, log(r) + log(e) + log(Time))
    l0prob_nb <- alpha * (lbeta - ldenom)
    zero_totalprob <- logplus(log(resulting_sample[it, 3]), log(1 - resulting_sample[it, 3]) + l0prob_nb)
    
    nd[it] <- rbinom(n = 1, size = Signal_0_len, prob = exp(log(resulting_sample[it, 3])) - zero_totalprob)
    
    if (((it - 1) %% 100) == 0) print(paste("iteration", it - 1))
    
    if (debug_mode) {
      print(list(omega = exp(lomega), 
                 Sigma_symmetric = isSymmetric(Sigma),
                 Sigma_positivesemidefinite = is.positive.semidefinite(Sigma),
                 accept_numlkernel = lhyperparam_kernel(lparams = proposal, pid = resulting_sample[it, 3], Signal = Signal),
                 accept_nummvnorm = dmvnorm(x = resulting_sample[it - 1, 1:2], mean = tau, sigma = Sigma, log = TRUE),
                 accept_denomlkernel = lhyperparam_kernel(lparams = resulting_sample[it - 1, 1:2], pid = resulting_sample[it, 3], Signal = Signal),
                 accept_denommvnorm = dmvnorm(x = proposal, mean = tau, sigma = Sigma, log = TRUE),
                 initial_state = resulting_sample[it - 1, 1:2], proposal = proposal, acceptance_ratio = exp(lacceptance_ratio), acception = accepted,
                 Signal = Signal, zeroprobability = exp(lpistar_tilt), pid = resulting_sample[it, 3], 
                 lmu = lalpha_tilti - lbeta_tilti, ltheta = lalpha_tilti - 2 * lbeta_tilti, alpha = exp(lalpha_tilt), beta = exp(lbeta_tilt), betai = exp(lbeta_tilti)))
...
set.seed(49)
L <- marginalised_gibbs_sampler(iterations = 10, D = emcee_results_list$simulated_data, A = A, Time = Time, a = a, r = R, e = e)
